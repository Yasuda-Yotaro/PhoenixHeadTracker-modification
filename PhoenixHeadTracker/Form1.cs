...
namespace PhoenixHeadTracker
{
    ...
    public partial class Form1 : Form
    {
        ...
        private IntPtr ptr;
        private float[] arr;
        private bool isRecording = false;       // 記録中かどうかを示すフラグ
        private List<string> imuLogData = new List<string>(); // ログデータをメモリに保存するリスト
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (isRecording)
            {
                ...
                string logLine = $"{timestamp},{arr[0]},{arr[1]},{arr[2]}";
                imuLogData.Add(logLine); // リストにログを追加
            }
            // ^^^ ここまで追加 ^^^
        ...
        public Form1()
        {
            ...
            // ▼▼▼ 以下を追加 ▼▼▼
            buttonRecordStart.Enabled = true;  // 開始ボタンを有効に
            buttonRecordStop.Enabled = false; // 停止ボタンを無効に
        }
        ...
        private void buttonRecord_Click_1(object sender, EventArgs e)
        {
            ...
            // ボタンの状態を更新
            buttonRecordStart.Enabled = false;
            buttonRecordStop.Enabled = true;

            // ログリストを初期化し、ヘッダー行を追加
            imuLogData.Clear();
            imuLogData.Add("Timestamp,Roll,Pitch,Yaw");
        }
        ...
        private void buttonRecordStop_Click(object sender, EventArgs e)
        {
            ...
            // ボタンの状態を更新
            buttonRecordStart.Enabled = true;
            buttonRecordStop.Enabled = false;

            // ファイル保存ダイアログを表示
            SaveFileDialog saveFileDialog = new SaveFileDialog();
            saveFileDialog.Filter = "CSVファイル (*.csv)|*.csv|すべてのファイル (*.*)|*.*";
            saveFileDialog.FileName = $"imu_log_{DateTime.Now:yyyyMMdd_HHmmss}.csv";
            saveFileDialog.Title = "ログファイルの保存先を選択";

            if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    // メモリに蓄積したデータを一括でファイルに書き込む
                    File.WriteAllLines(saveFileDialog.FileName, imuLogData);
                    MessageBox.Show("ログを保存しました。", "保存完了", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                catch (Exception ex)
                {
                    MessageBox.Show($"ファイルの保存に失敗しました: {ex.Message}");
                }
            }
        }
